...
- 由于上游 chart 不支持额外 volumeMount，本仓库将 `prometheus-smartctl-exporter` 0.17.1 chart 本地化（vendored），扩展 `extraVolumes` / `extraVolumeMounts`，并注入 wrapper 将 NVMe 的 `--log=error` 改写为 `--log=error,64`。
